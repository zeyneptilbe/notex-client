...
function validateCommentFiles(files: File[]): {
  valid: File[];
  errors: string[];
} {
  const valid: File[] = [];
  ...
  for (const file of files) {
    ...
    if (file.size > MAX_COMMENT_FILE_SIZE) {
      errors.push(`"${file.name}" — dosya boyutu 50MB limitini aşıyor`);
      continue;
    }
    valid.push(file);
  }
...
export default function PostDetail() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        ...
        try {
          const authorRes = await postsApi.getAll({
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (commentFiles.length > 0) {
        ...
        if (failed.length > 0) {
          showToast(`Bazı dosyalar yüklenemedi: ${failed.join(", ")}`, "error");
        }
      }
  ...
  const handleAddReply = async (parentId: string) => {
    ...
    try {
      ...
      const replyWithAttachments = {
        ...newReply,
        attachments: [...(newReply.attachments || []), ...uploadedAttachments],
      };
  ...
  const handleDeleteComment = async (commentId: string, parentId?: string) => {
    if (
      ...
      !hasPermission("comments.deleteOthers")
    ) {
      showToast("Bu işlem için yetkiniz bulunmuyor.", "warning");
      return;
    }
        ...
        <div className="flex-1 min-w-0">
          ...
          <article className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="px-8 py-8 border-b border-gray-100">
              <h1 className="text-3xl font-bold text-gray-800 mb-5 wrap-break-word">
                {post.title}
              </h1>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {((isOwner && hasPermission("posts.update")) ||
                    hasPermission("posts.updateOthers")) && (
                    <button
                      onClick={handleEdit}
                  ...
                  {((isOwner && hasPermission("posts.delete")) ||
                    hasPermission("posts.deleteOthers")) && (
                    <button
            ...
            <div className="px-8 py-8">
              ...
              {post.summary && (
                <div className="mb-8 p-5 bg-gray-50 rounded-lg border-l-4 border-blue-500">
                  <p className="text-gray-700 italic text-base leading-relaxed wrap-break-word">
                    {post.summary}
                  </p>
                </div>
              )}
              ...
              <div className="prose prose-lg prose-gray max-w-none wrap-break-word">
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-600 text-sm mb-2 wrap-break-words">
                          {comment.content}
                        </p>
                            ...
                            {comment.replies.map((reply) => (
                              <div key={reply.id} className="flex gap-3">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-gray-600 text-sm wrap-break-words">
                                    {reply.content}
                                  </p>
                                  {/* Yanıt ekleri */}
                                  {reply.attachments &&
                                            ...
                                            >
                                              ...
                                              <div className="flex items-center gap-1 shrink-0 ml-2">
                                                <button
                                                  type="button"
                                                  onClick={() =>
                                                    commentAttachmentsApi.download(
                                                      att.id,
                                                    )
                                                  }
                                                  className="px-2 py-0.5 text-blue-600 hover:bg-blue-50 rounded font-medium transition-colors"
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-600">
            <strong>{post.title}</strong> postunu reddetmek istediğinize emin
            misiniz?
          </p>
          <div>
